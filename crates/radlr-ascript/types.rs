use radlr_core::{parser::ASTNode, proxy::OrderedMap, *};
...
pub struct AScriptStruct {
  ...
  /// that has a [radlr_rust_runtime::type::Token] value comprised of the
...
impl AScriptStore {
  pub fn dummy() -> RadlrResult<Self> {
    RadlrResult::Ok(AScriptStore {
  ...
  pub fn new(mut j: Journal, db: &ParserDatabase) -> RadlrResult<Self> {
