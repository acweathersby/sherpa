...
type StructConstructorExpr =
  dyn Fn(&AscriptWriterUtils, &mut CodeWriter<Vec<u8>>, String, Vec<(String, String, AScriptTypeVal)>, bool) -> RadlrResult<()>;
...
impl<'a, W: Write> AscriptWriter<'a, W> {
  ...
  pub fn block(
    ...
    open_delim: &str,
    closing_delim: &str,
    content_writer: &dyn Fn(&mut Self) -> RadlrResult<()>,
  ) -> RadlrResult<()> {
  ...
  pub fn stmt(&mut self, stmt: String) -> RadlrResult<()> {
  ...
  pub fn method(
    ...
    open_delim: &str,
    closing_delim: &str,
    content_writer: &mut dyn FnMut(&mut Self) -> RadlrResult<()>,
  ) -> RadlrResult<()> {
    self.writer.write_line("\n")?;
  ...
  pub fn list<S: Display>(&mut self, delim: &str, data: Vec<S>) -> RadlrResult<()> {
    for datum in data {
  ...
  pub fn write_struct_data<'b: 'a>(
    &mut self,
    struct_write_script: &dyn Fn(&mut Self, &StructData) -> RadlrResult<()>,
  ) -> RadlrResult<()> {
  ...
  fn write_node_token(&mut self, rule: &Rule) -> RadlrResult<()> {
  ...
  pub fn write_reduce_functions(
    ...
    open_delim: &str,
    closing_delim: &str,
    reduce_fn_writer: &dyn Fn(&mut Self, &Vec<String>) -> RadlrResult<()>,
  ) -> RadlrResult<()> {
    let store = self.store;
    let db = self.db;
    let rules = db.rules();
      ...
      #[cfg(debug_assertions)]
      {
        if nterm_data.len() != 1 {
          unreachable!(
            ...
            rule.tok.blame(1, 1, "", radlr_rust_runtime::types::BlameColor::RED),
