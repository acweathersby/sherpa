...
/// configuration, monitoring, and reporting of Radlr commands and types.
pub struct Journal {
...
impl Journal {
  ...
  pub fn errors_since_last_check() -> Option<Vec<RadlrError>> {
    None
  }
  ...
  pub fn get_faulty_reports(&self) -> Option<Vec<Report>> {
    ...
    self.get_reports(ReportType::Any, move |report| {
      if report.have_errors_of_type(RadlrErrorSeverity::Critical) {
  ...
  /// Consumes all reports and returns any errors generated by the reports.
  pub fn extract_errors(&mut self) -> Vec<RadlrError> {
    let mut errors = vec![];
    match self.global_pad.write() {
      LockResult::Ok(mut global_pad) => {
        for (_, report) in global_pad.reports.drain() {
          errors.extend(report._errors.into_values())
        }
      }
      Err(err) => {
        panic!("{err}");
