...
fn create_out_of_scope_complete_state<'a, T: TransitionPairRefIter<'a> + Clone>(
  ...
  if let Some(i) = out_of_scope.clone().next() {
    ...
    GraphNodeBuilder::new()
      .set_sym(sym)
      .set_type(transition_type)
      .set_kernel_items(out_of_scope.to_kernel().cloned())
        ...
        ____allow_rd____.then(|| create_call(gb, pred, in_scope.clone(), prec_sym)).flatten()
      {
        ...
        if group.iter().all(|p| p.is_kernel_terminal()) {
          GraphNodeBuilder::new()
            ...
            .set_type(StateType::KernelShift)
            .set_kernel_items(group.iter().to_kernel().try_increment().into_iter())
            .commit(gb);
        } else if len == 1 {
          ...
          if !____allow_lr____ {
            if let Some(non_term) = kernel_item.nonterm_index_at_sym(pred.graph_type(), db) {
              ...
              if gb.db().nonterm_recursion_type(non_term).is_left_recursive() {
                ...
                let from = GraphNodeBuilder::new()
                  .set_sym(prec_sym)
                  .set_parent(pred.clone())
                  .set_build_state(GraphBuildState::Normal)
                  .set_type(StateType::Shift)
                  .set_kernel_items(items.into_iter())
                  .commit(gb)?;

                GraphNodeBuilder::new()
                  .set_sym((kernel_item.sym_id(db), 0).into())
                  .set_parent(pred.clone())
                  .set_build_state(GraphBuildState::Normal)
                  .set_type(StateType::ShiftFrom(from.id()))
                  .set_kernel_items([kernel_item.try_increment()].into_iter())
                  .commit(gb)?;
              }
            } else {
              GraphNodeBuilder::new()
      ...
      if ____is_scan____ {
        ...
        match resolve_reduce_reduce_conflict(gb, pred, prec_sym, lookahead_pairs)? {
          ...
          ReduceReduceConflictResolution::Fork(lookahead_pairs) => {
            create_fork(gb, pred, prec_sym, lookahead_pairs.iter().map(|i| i.kernel))?.to_pending(gb);
          }
          ...
          ReduceReduceConflictResolution::Peek(max_k, follow_pairs) => {
            let state = create_peek(gb, pred, prec_sym, [].iter(), Some(follow_pairs.iter()))?;
            state.to_pending(gb);
          }
        }
      }
      ...
      if ____is_scan____ {
        ...
        match resolve_shift_reduce_conflict(gb, pred, group.iter(), lookahead_pairs.iter())? {
          ...
          ShiftReduceConflictResolution::Peek(max_k) => {
            create_peek(gb, pred, prec_sym, group.iter(), Some(lookahead_pairs.iter()))?.to_pending(gb);
          }
