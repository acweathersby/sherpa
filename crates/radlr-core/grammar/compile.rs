...
fn compile_grammar_data(j: &mut Journal, g_data: GrammarData, g_s: &GrammarSoup) -> RadlrResult<GrammarIdentities> {
  let id = g_data.id;
...
fn load_from_str(j: &mut Journal, source: &str, source_path: PathBuf, soup: &GrammarSoup) -> RadlrResult<GrammarData> {
