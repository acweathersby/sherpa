...
use radlr_core::{RadlrDatabase, RadlrGrammar, RadlrGraph, *};
use radlr_rust_build::build_rust;
...
/// A Parser database derived from grammar defined in a JSSoup
#[wasm_bindgen]
pub struct JSParserGraph(pub(crate) Box<RadlrGraph>);

impl AsRef<RadlrGraph> for JSParserGraph {
  fn as_ref(&self) -> &RadlrGraph {
    &self.0
  }
}
...
pub fn create_rust_ast_output(js_db: &JSParserDB) -> Result<String, PositionedErrors> {
  ...
  j.set_active_report("ast compile", radlr_core::ReportType::Any);
...
pub fn create_parser_states(
  ...
  j.set_active_report("state construction", radlr_core::ReportType::Any);
...
pub fn create_bytecode(states: &JSIRParser) -> Result<JSBytecodeParserDB, PositionedErrors> {
  ...
  j.set_active_report("bytecode compile", radlr_core::ReportType::Any);

  let RadlrResult::Ok(pkg) = compile_bytecode(states.states.as_ref(), true) else {
    return Result::Err(convert_journal_errors(&mut j));
  };
