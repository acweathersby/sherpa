...
use super::{parser_db::ParserDatabase, *};
use radlr_core_common::{code_writer::CodeWriter, utils::create_u64_hash, CachedString};
...
#[repr(u8)]
pub enum SymbolId {
  /// A symbol reference that is in the process of being parsed and compiled
  /// into a more specific symbol type.
  Pending(usize),
  Undefined,
  Default,
  EndOfFile,
  ...
  DBNonTerminalToken {
    nonterm_key: DBNonTermKey,
    sym_key:     Option<DBTermKey>,
  },
  DBNonTerminal {
    key: DBNonTermKey,
  },
...
impl SymbolId {
  ...
  pub fn debug_string(&self, db: &ParserDatabase) -> String {
    ...
    match *self {
      ...
      Any => &mut w + "c:any",
      Token { val } => &mut w + " " + val.to_str().as_str() + " ",
...
impl Debug for SymbolId {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    use SymbolId::*;
    match *self {
      Undefined => f.write_str("Undefine")?,
      Default => f.write_str("Default")?,
      EndOfFile { .. } => f.write_str("{EOF}")?,
      ClassSpace => f.write_str("c:sp")?,
      ClassHorizontalTab => f.write_str("c:tab")?,
      ClassNewLine => f.write_str("c:nl")?,
      ClassIdentifier => f.write_str("c:id")?,
      ClassNumber => f.write_str("c:num")?,
      ClassSymbol => f.write_str("c:sym")?,
      Any => f.write_str("c:any")?,
      Token { val } => f.write_str(val.to_str().as_str())?,
      Codepoint { val } => f.write_str(&char::from_u32(val).unwrap().to_string())?,
      DBNonTerminal { key } => key.fmt(f)?,
      DBNonTerminalToken { nonterm_key: nterm_key, .. } => {
        nterm_key.fmt(f)?;
      }
      DBToken { key: index } => index.fmt(f)?,
      Char { char } => f.write_str(&char::from_u32(char as u32).unwrap().to_string())?,
      Pending(val) => f.write_fmt(format_args!("pending[{val}]"))?,
    };
