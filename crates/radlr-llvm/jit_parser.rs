...
where
  ...
  ASTNode: AstObject,
{
  pub(crate) fn new<'db>(store: RadlrParserBuilder, ctx: &'llvm Context) -> RadlrResult<Self> {
    unsafe {
      ...
      engine.add_global_mapping(&llvm_mod.fun.allocate_stack, radlr_allocate_stack as usize);
      engine.add_global_mapping(&llvm_mod.fun.free_stack, radlr_free_stack as usize);
      engine.add_global_mapping(&llvm_mod.fun.get_token_class_from_codepoint, radlr_get_token_class_from_codepoint as usize);
  ...
  ASTNode: AstObject,
{
  pub(crate) fn init(&mut self) {
    ...
    unsafe {
      let Self { ctx: radlr_ctx, reader, .. } = self;
      self.init_fn.call(radlr_ctx, *reader);
    }
  }
...
impl<'llvm, R: ByteReader + LLVMByteReader + MutByteReader, M> RadlrParser<R, M, true> for JitParser<'llvm, R, M> {
