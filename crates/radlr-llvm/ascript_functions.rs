use crate::{build_fast_call, CtxAggregateIndices, LLVMParserModule, LLVMTypes};
use inkwell::{
  module::Linkage,
  types::StructType,
...
use radlr_core::*;
use radlr_rust_runtime::{deprecate::*, types::ParseActionType};

pub(crate) unsafe fn construct_ast_builder<ASTNode: AstObject>(module: &LLVMParserModule) -> RadlrResult<()> {
  ...
  // - b.build_store(slot_direction, i8.const_int(1, false)); // Directory of slot
  //   is decreasing
  ...
  // Get slice size
  let symbol_count_original = CtxAggregateIndices::sym_len.load(b, parse_ctx)?.into_int_value();
  let rule_index = CtxAggregateIndices::rule_id.load(b, parse_ctx)?.into_int_value();
