...
use radlr_core::*;
use radlr_rust_runtime::types::ParseActionType;
...
pub(crate) fn construct_module<'a>(ctx: &'a Context, target_data: &TargetData, module: Module<'a>) -> LLVMParserModule<'a> {
  ...
  GOTO.set_body(&[GOTO_STACK_SPEC_FUNCTION.ptr_type(0.into()).into(), i32.into(), i32.into()], false);
  ...
  let get_input_info_fun = ctx
    .bool_type()
    .fn_type(&[READER.ptr_type(0.into()).into(), i8_ptr_ptr, i8_ptr_ptr, i8_ptr_ptr, i8_ptr_ptr, i8_ptr_ptr, i8_ptr_ptr], false)
    .ptr_type(0.into());
...
pub(crate) unsafe fn construct_emit_end_of_parse(module: &LLVMParserModule) -> RadlrResult<()> {
  ...
  if funct.handle_eop.verify(true) {
    ...
    RadlrResult::Err(RadlrError::from("\n\nCould not build emit_eop function"))
  }
}

pub(crate) unsafe fn construct_get_adjusted_input_block_function(m: &LLVMParserModule) -> RadlrResult<()> {
  ...
  let input_complete = b
    .build_call(
      CallableValue::try_from(CtxAggregateIndices::get_input_info.load(b, p_ctx)?.into_pointer_value())?,
      &[
...
pub(crate) unsafe fn construct_drop(module: &LLVMParserModule) -> RadlrResult<()> {
  let LLVMParserModule { b, ctx, fun: funct, .. } = module;
...
pub(crate) unsafe fn construct_internal_free_stack(module: &LLVMParserModule) -> RadlrResult<()> {
  ...
  let goto_byte_size = b.build_left_shift(goto_slot_count, ctx.i32_type().const_int(4, false), "goto_byte_size");
  ...
  let goto_free_bytes = b.build_left_shift(goto_free, ctx.i32_type().const_int(4, false), "remaining_bytes");
...
pub(crate) unsafe fn construct_extend_stack(module: &LLVMParserModule) -> RadlrResult<()> {
  ...
  let new_slot_byte_size = b.build_left_shift(new_slot_count, ctx.i32_type().const_int(4, false), "total_bytes");
  ...
  let new_stack_top_ptr = b.build_int_to_ptr(new_stack_top_ptr, types.goto.ptr_type(0.into()), "new_top");
...
pub(crate) unsafe fn construct_init(module: &LLVMParserModule) -> RadlrResult<()> {
  let LLVMParserModule { b, ctx, fun: funct, .. } = module;
  let i32 = ctx.i32_type();
  ...
  CtxAggregateIndices::state.store(b, parse_ctx, i32.const_int(NORMAL_STATE_FLAG_LLVM as u64, false))?;
  b.build_return(None);

  validate(fn_value)
}

pub(crate) unsafe fn construct_pop_function(module: &LLVMParserModule) -> RadlrResult<()> {
  let LLVMParserModule { b, ctx, fun: funct, .. } = module;
...
pub(crate) fn construct_push_state_function(module: &LLVMParserModule) -> RadlrResult<()> {
  let LLVMParserModule { b, types, ctx, fun: funct, .. } = module;
  let i32 = ctx.i32_type();
...
pub(crate) fn construct_utf8_lookup_function(module: &LLVMParserModule) -> RadlrResult<()> {
  ...
  let codepoint_info = b.build_insert_value(module.types.cp_info.get_undef(), zero, 0, "cp_info").unwrap();
  ...
  let bit_count = b
    .build_call(funct.ctlz_i8, &[invert.into(), bool.const_int(0, false).into()], "header_bit_count")
    .try_as_basic_value()
    .unwrap_left()
    .into_int_value();
  let comparison = b.build_int_compare(inkwell::IntPredicate::EQ, bit_count, i8.const_int(0, false), "");
  ...
  let codepoint_info = b.build_insert_value(codepoint_info_base, b.build_int_z_extend(byte, i32, ""), 0, "").unwrap();
  // Insert the codepoint byte length into the CP_INFO struct
  let codepoint_info = b.build_insert_value(codepoint_info, i32.const_int(1, false), 1, "").unwrap();
...
pub(crate) unsafe fn construct_merge_utf8_part_function(module: &LLVMParserModule) -> RadlrResult<()> {
  let i32 = module.ctx.i32_type();
  let b = &module.b;
  let funct = &module.fun;
...
pub(crate) unsafe fn construct_next_function<'a>(module: &'a LLVMParserModule) -> RadlrResult<()> {
...
pub(crate) unsafe fn construct_dispatch_functions<'a>(m: &'a LLVMParserModule) -> RadlrResult<()> {
...
/// Compile a LLVM parser module from Radlr bytecode.
pub fn compile_llvm_module_from_parse_states<'llvm, T: ParserStore>(
  ...
  module
    .fun
    .push_state
    .add_attribute(inkwell::attributes::AttributeLoc::Function, module.ctx.create_string_attribute("alwaysinline", ""));
  RadlrResult::Ok(())
}
pub fn validate(func: FunctionValue) -> RadlrResult<()> {
  if func.verify(true) {
    RadlrResult::Ok(())
  } else {
    RadlrResult::Err(RadlrError::from(format!(
      "\n\nCould not validate {} \n{}",
