...
import { DebuggerButton, DebuggerCheckbox } from "./debugger_io";
...
class InitCodeMirror extends FlowNode<DebuggerData> {
  update(t: string, state: DebuggerData) {
    ...
    if (t == "init") {
      ...
      state.parser_host.setText(get_input())
...
class ParseBuilder extends FlowNode<DebuggerData> {
  ...
  private setConfig() {
    ...
    this.config.ALLOW_CALLS = DebuggerCheckbox.get("rd-enable").ele.checked;
    this.config.ALLOW_CONTEXT_SPLITTING = DebuggerCheckbox.get("fk-enable").ele.checked;
...
export function initDebugger(
  grammar_ctx: GrammarContext, parser_host: CMThing,
  ...
  DebuggerCheckbox.get("rd-enable").ele.checked = default_config.ALLOW_CALLS;
  DebuggerCheckbox.get("fk-enable").ele.checked = default_config.ALLOW_CONTEXT_SPLITTING;
