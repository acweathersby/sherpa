...
use sherpa_core::{SherpaError, SherpaResult};
...
/// ```
/// mod my_parser {
/// # use sherpa_rust_build::compile_mod as compile;
///   compile! {
///     IGNORE { c:sp }
///
...
pub fn build_rust(db: &ParserDatabase, store: &AScriptStore) -> SherpaResult<String> {
  let u: crate::output_base::AscriptWriterUtils<'_> = create_rust_writer_utils(&store, &db);
