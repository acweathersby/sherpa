...
use radlr_core::{ParserStore, RadlrError, RadlrResult};
use radlr_rust_runtime::{deprecate::*, types::Token};
...
pub fn build_llvm_parser<T: ParserStore>(
  ...
  let radlr_mod = &construct_module(&ctx, &target_data, module);

  let opt = OptimizationLevel::Aggressive;

  compile_llvm_module_from_parse_states(parse_store, radlr_mod)?;

  if enable_ascript {
    unsafe {
      construct_ast_builder::<DummyASTEnum>(&radlr_mod).unwrap();
    }
  }

  apply_llvm_optimizations(opt, &radlr_mod);

  if let Ok(mut file) = std::fs::File::create(&ll_file_path) {
    file.write_all(radlr_mod.module.to_string().as_bytes())?;
    file.flush()?;
  }

  match target_machine.write_to_file(&radlr_mod.module, FileType::Object, &object_path) {
    Ok(_) => {
      if !(Command::new(ar_command)
        ...
        .success())
      {
        RadlrResult::Err(RadlrError::from("Unable to compile llvm bitcode: Incorrect path to LLVM-AR executable"))
      } else {
        RadlrResult::Ok(())
      }
    }
    Err(err) => RadlrResult::Err(RadlrError::from(err.to_string())),
  }
}
