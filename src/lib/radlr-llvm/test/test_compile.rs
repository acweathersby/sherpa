#![allow(unused)]
use crate::{ascript_functions::*, jit_parser::JitParser, parse_functions::compile_states, *};
use inkwell::{
  context::Context,
...
use radlr_core::{proxy::OrderedMap, test::utils::build_parse_states_from_source_str, *};
use radlr_rust_runtime::{
  deprecate::*,
  llvm_parser::{radlr_allocate_stack, radlr_free_stack, radlr_get_token_class_from_codepoint, LLVMByteReader},
...
type Init<R = UTF8StringReader<'static>, T = u32> = unsafe extern "C" fn(*mut ParseContext<R, T>, *mut R);
type PushState<R = UTF8StringReader<'static>, T = u32> = unsafe extern "C" fn(*mut ParseContext<R, T>, u32, usize);

type Next<R = UTF8StringReader<'static>, T = u32> = unsafe extern "C" fn(*mut ParseContext<R, T>) -> ParseActionType;

type Prime<R = UTF8StringReader<'static>, T = u32> = unsafe extern "C" fn(*mut ParseContext<R, T>, u32);

type Extend<R = UTF8StringReader<'static>, T = u32> = unsafe extern "C" fn(*mut ParseContext<R, T>, u32);
...
fn build_fast_call_shim<'a>(module: &LLVMParserModule<'a>, fun: inkwell::values::FunctionValue<'a>) -> RadlrResult<()> {
  ...
  let result = build_fast_call(&module.b, fun, &shim.get_params().into_iter().map(|i| i.into()).collect::<Vec<_>>())?;
...
#[test]
fn verify_construction_of_init_function() -> RadlrResult<()> {
...
#[test]
fn verify_construction_of_ast_builder() -> RadlrResult<()> {
  let context = Context::create();
  let target_machine = crate_target_test_machine()?;
  let target_data = target_machine.get_target_data();
  let module = construct_module(&context, &target_data, context.create_module("test"));
...
#[test]
fn verify_construction_of_push_state_function() -> RadlrResult<()> {
  let context = Context::create();
  let target_machine = crate_target_test_machine()?;
  let target_data = target_machine.get_target_data();
  let module = construct_module(&context, &target_data, context.create_module("test"));
...
#[test]
fn should_push_new_state() -> RadlrResult<()> {
  let context = Context::create();
  let target_machine = crate_target_test_machine()?;
  let target_data = target_machine.get_target_data();
  ...
  unsafe {
    ...
    jit_engine.add_global_mapping(&module.fun.allocate_stack, radlr_allocate_stack as usize);
    jit_engine.add_global_mapping(&module.fun.free_stack, radlr_free_stack as usize);
    ...
    let stack = std::slice::from_raw_parts(
      { (rt_ctx.goto_stack_ptr as usize - ((rt_ctx.goto_size - rt_ctx.goto_free) << 4) as usize) as *mut Goto },
...
#[test]
fn verify_construction_of_get_adjusted_input_block_function() -> RadlrResult<()> {
  let context = Context::create();
  let target_machine = crate_target_test_machine()?;
  let target_data = target_machine.get_target_data();
  let module = construct_module(&context, &target_data, context.create_module("test"));
...
#[test]
fn verify_construction_of_emit_eop_function() -> RadlrResult<()> {
  let context = Context::create();
  let target_machine = crate_target_test_machine()?;
  let target_data = target_machine.get_target_data();
  let module = construct_module(&context, &target_data, context.create_module("test"));
...
#[test]
fn should_initialize_context() -> RadlrResult<()> {
  let context = Context::create();
  let target_machine = crate_target_test_machine()?;
  let target_data = target_machine.get_target_data();
  let module = construct_module(&context, &target_data, context.create_module("test"));
...
#[test]
fn verify_utf8_lookup_functions() -> RadlrResult<()> {
  let context = Context::create();
  let target_machine = crate_target_test_machine()?;
  let target_data = target_machine.get_target_data();
  let module = construct_module(&context, &target_data, context.create_module("test"));
...
#[test]
fn should_yield_correct_cp_values_for_inputs() -> RadlrResult<()> {
  let context = Context::create();
  let target_machine = crate_target_test_machine()?;
  let target_data = target_machine.get_target_data();
  let module = construct_module(&context, &target_data, context.create_module("test"));
...
#[test]
fn verify_construct_extend_stack() -> RadlrResult<()> {
  let context = Context::create();
  let target_machine = crate_target_test_machine()?;
  let target_data = target_machine.get_target_data();
  let module = construct_module(&context, &target_data, context.create_module("test"));
...
#[test]
fn should_extend_stack() -> RadlrResult<()> {
  let context = Context::create();
  let target_machine = crate_target_test_machine()?;
  let target_data = target_machine.get_target_data();
  let module = construct_module(&context, &target_data, context.create_module("test"));
  ...
  unsafe {
    ...
    let stack = {
      let ptr = (rt_ctx.goto_stack_ptr as usize) - (((rt_ctx.goto_size as usize) - (rt_ctx.goto_free as usize)) << 4);
...
#[test]
fn test_compile_parse_function() -> RadlrResult<()> {
...
#[test]
fn test_compile_from_bytecode() -> RadlrResult<()> {
  build_parse_states_from_source_str(
    "
IGNORE { c:sp }
  ...
  ",
    ...
    &|tp| {
      ...
      unsafe {
        ...
        engine.add_global_mapping(&module.fun.allocate_stack, radlr_allocate_stack as usize);

        engine.add_global_mapping(&module.fun.free_stack, radlr_free_stack as usize);

        engine.add_global_mapping(&module.fun.get_token_class_from_codepoint, radlr_get_token_class_from_codepoint as usize);
...
#[test]
fn line_tracking_with_scanner_tokens() -> RadlrResult<()> {
...
#[test]
fn test_compile_from_bytecode1() -> RadlrResult<()> {
  let ctx = Context::create();
...
#[test]
fn test_compile_json_parser() -> RadlrResult<()> {
...
fn crate_target_test_machine() -> RadlrResult<TargetMachine> {
  ...
  RadlrResult::Ok(target.create_target_machine(&native_triple, "generic", "", opt, reloc, model).unwrap())
}
