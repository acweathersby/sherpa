...
impl CtxAggregateIndices {
  pub fn get_ptr<'a>(&self, b: &Builder<'a>, parse_ctx: PointerValue<'a>) -> RadlrResult<PointerValue<'a>> {
    RadlrResult::Ok(b.build_struct_gep(parse_ctx, (*self).into(), &format!("{:?}_ptr", self))?)
  }

  pub fn load<'a>(&self, b: &Builder<'a>, parse_ctx: PointerValue<'a>) -> RadlrResult<BasicValueEnum<'a>> {
    let val = b.build_load(self.get_ptr(b, parse_ctx)?, &format!("{:?}_val", self));

    RadlrResult::Ok(val)
  }

  pub fn load_ptr_as_int<'a>(&self, b: &Builder<'a>, parse_ctx: PointerValue<'a>, int: IntType<'a>) -> RadlrResult<IntValue<'a>> {
    let val = b.build_load(self.get_ptr(b, parse_ctx)?, &format!("{:?}_val", self)).into_pointer_value();
    RadlrResult::Ok(b.build_ptr_to_int(val, int, &format!("{:?}_val", self)).into())
  }

  pub fn store<'a, V>(&self, b: &Builder<'a>, parse_ctx: PointerValue<'a>, value: V) -> RadlrResult<()>
  where
    V: BasicValue<'a>,
  {
