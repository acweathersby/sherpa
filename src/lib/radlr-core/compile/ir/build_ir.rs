...
pub fn build_ir_from_graph(graph: &RadlrGraph) -> RadlrResult<(ParserClassification, ParseStatesMap)> {
  let RadlrGraph { j, db, config, parsers, scanners } = graph;
...
fn convert_nonterm_shift_state_to_ir<'graph: 'graph>(
...
) -> RadlrResult<(StateId, Box<ParseState>)> {
  ...
  RadlrResult::Ok((state.get_id(), goto))
}
