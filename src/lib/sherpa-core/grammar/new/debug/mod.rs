...
use crate::{
  grammar::new::{bytecode::disassemble_parse_block, compile::ItemRef},
  test::utils::PrintConfig,
  SherpaResult,
};
use sherpa_runtime::{bytecode_parser::DebugEvent, types::bytecode::Opcode};
...
pub fn console_debugger<'a>(
  ...
  {}
  -------------------------------------------------------------------------------",
          disassemble_parse_block(Some(*base_instruction), Some(&db), base_instruction.bytecode()).0
        );
      println!("");
    }
  ...
  -------------------------------------------------------------------------------",
            ...
            &string[(*active_ptr)..(active_ptr + input_window_size).min(string.len())]
              .replace("\n", "\\n"),
            disassemble_parse_block(Some(*instruction), Some(&db), instruction.bytecode()).0
          );
        println!("");
      }
    }
