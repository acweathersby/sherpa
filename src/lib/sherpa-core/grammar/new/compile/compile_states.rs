...
use crate::{
  ...
  grammar::{
    ...
    hash_id_value_u64,
    new::types::{CompileDatabase, OrderedMap, ProductionSubType, Rule},
  },
...
pub(crate) async fn compile_parse_states(mut j: Journal, db: &CompileDatabase) {
  j.set_active_report(
    "test",
  ...
  for (prod_id, prod_sym) in db.productions().iter().enumerate() {
    ...
    match prod_sym {
      SymbolId::NonTerminal { id } => {
        //Run parser pass
        let graph =
          create_graph(&mut j, GraphMode::Parser, start_items, db, &follow)
            .unwrap();
        ...
        // Then for each ir calculate scanner set. If set is unique build
        // scanner
      }
      SymbolId::NonTerminalToken { id, .. } => {
        //Run scanner
        let graph =
          create_graph(&mut j, GraphMode::Scanner, start_items, db, &follow)
            .unwrap();
...
fn create_follow_sets<'db>(db: &'db CompileDatabase) -> FollowSets<'db> {
...
pub(super) fn create_graph<'db>(
  ...
  let root =
    graph.create_state(SymbolId::Default, StateType::Start, None, kernel_items);
...
fn handle_kernel_items(
  ...
  let out_items =
    handle_incomplete_items(j, graph, parent, graph_state, groups)?;
...
fn handle_goto<'db>(
  ...
  if !non_terminals.is_empty() && !out_items.is_empty() {
    ...
    let mut queue =
      VecDeque::from_iter(out_items.iter().map(|i| i.prod_index()));

    while let Some(prod_id) = queue.pop_front() {
      if seen.insert(prod_id) {
        for item in non_terminals
          .iter()
          .filter(|i| i.prod_index_at_sym().unwrap() == prod_id)
        {
    ...
    if !used_non_terms.is_empty() {
      ...
      let used_goto_groups = hash_group_btreemap(used_non_terms, |_, t| {
        t.prod_index_at_sym().unwrap_or_default()
      });

      for (prod_id, items) in &used_goto_groups {
        let sym_id = db.prod_sym(*prod_id);
        ...
        if kernel_prod_ids.remove(&prod_id)
          ...
          && completed.is_empty()
        {
          let item = ItemRef::from_rule(db.prod_rules(*prod_id)?[0], db);
...
fn create_peek<'a, 'db: 'a, T: ItemContainerIter<'a, 'db>>(
  graph: &mut Graph<'db>,
  sym: SymbolId,
  parent: StateId,
  ...
  let state =
    graph.create_state(sym, transition_type, Some(parent), Array::default());
      ...
      .filter_map(|FollowPair { follow, .. }| {
        if follow.is_complete() || {
          existing_items.contains(&follow)
            || (follow.at_start()
              && existing_prod_ids.contains(&follow.prod_index()))
        } {
          None
        } else {
          Some(*follow)
...
fn get_kernel_items_from_peek<'db>(
  graph: &Graph<'db>,
  peek_item: &ItemRef<'db>,
) -> Items<'db> {
...
fn create_call<'db>(
  ...
  if all_items_come_from_same_production_call(group) {
    ...
    if !graph[parent].conflicting_production_call(prod_id, is_scan, db) {
      ...
      if items.len() > 0 {
        if let Some(items) =
          create_call(&items, graph, graph_state, parent, sym, is_scan)
        {
          return Some(items);
        } else {
...
fn resolve_conflicting_symbols<'db>(
  j: &mut Journal,
  graph: &mut Graph<'db>,
  par: StateId,
  sym: SymbolId,
  ...
  let symbol_groups = hash_group_btreemap(completed_items.clone(), |_, i| {
    i.origin.get_symbol(graph.get_db())
  });
  let priority_groups =
    hash_group_btreemap(symbol_groups, |_, (sym, _)| match sym {
      SymbolId::IndexedToken { .. } => Defined,
      _ => Generic,
    });
...
fn create_out_of_scope_complete_state<'db>(
...
fn get_set_of_occluding_items<'db>(
  j: &mut Journal,
  into_sym: &SymbolId,
  ...
  occluding
    .iter()
    .filter(|i| i.precedence() >= precedence)
    .cloned()
    .collect::<ItemSet>()
}

fn create_transition_groups<'db>(
  graph: &mut Graph<'db>,
  parent: StateId,
  ...
  let mut groups = hash_group_btreemap(
    closure
      .iter()
      .filter(|i| i.precedence() >= max_precedence)
      .cloned()
      .collect::<ItemSet>(),
    |_, item| match item.get_type() {
...
fn get_completed_item_artifacts<'a, 'db: 'a, T: ItemContainerIter<'a, 'db>>(
  j: &mut Journal,
  graph: &mut Graph<'db>,
  par: StateId,
  completed: T,
    ...
    if completed.insert(item) {
      ...
      let closure = if item.is_out_of_scope() {
        graph[item.origin_state]
          ...
          .filter(|i| {
            i.is_out_of_scope()
              && i.prod_index_at_sym().unwrap_or_default() == prod_id
          })
...
fn get_goal_items_from_completed<'db>(
  items: &Items<'db>,
  graph: &Graph<'db>,
) -> ItemSet<'db> {
...
fn get_kernel_items_from_peek_item<'db>(
  graph: &Graph<'db>,
  peek_item: &ItemRef<'db>,
) -> Items<'db> {
  let Origin::Peek(peek_index, peek_origin) = peek_item.origin else {
...
fn resolve_peek<'a, 'db: 'a, T: ItemContainerIter<'a, 'db>>(
  graph: &mut Graph<'db>,
  mut completed: T,
  sym: SymbolId,
  par: StateId,
  ...
  let kernel_items =
    get_kernel_items_from_peek(graph, completed.next().unwrap());
  let state =
...
fn handle_completed_item<'db>(
  j: &mut Journal,
  graph: &mut Graph<'db>,
  (completed_item, completed_items): (ItemRef<'db>, Items<'db>),
  parent: StateId,
  sym: SymbolId,
  graph_state: GraphState,
) -> SherpaResult<()> {
  ...
  match (completed_item.origin, is_scan) {
    ...
    _ => {
      let state = graph.create_state(
        sym,
        StateType::Reduce(completed_item.prod_index()),
        Some(parent),
        vec![],
      );
      graph[state].set_reduce_item(completed_item);
