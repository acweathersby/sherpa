...
use super::item::{ItemContainer, ItemRef, ItemSet, ItemType, Items};
...
pub const OutScopeIndex: u32 = 0xFEEDDEED;
...
pub struct State<'db> {
  ...
  non_terminals: OrderedSet<ItemRef<'db>>,
...
impl<'db> State<'db> {
  ...
  pub fn set_transition_type(&mut self, transition_type: StateType) {
  ...
  pub fn set_non_terminals(&mut self, non_terms: &BTreeSet<ItemRef<'db>>) {
  ...
  pub fn num_of_kernel_items(&self) -> usize {
  ...
  pub fn get_reduce_item(&self) -> Option<ItemRef<'db>> {
    self.reduce_item
  }
...
impl<'follow, 'db: 'follow> Graph<'follow, 'db> {
  ...
  pub fn item_is_goal(&self, item: &ItemRef<'db>) -> bool {
    if !item.is_complete() {
      return false;
    }
