...
fn reduce<'a, 'llvm: 'a, 'db: 'a>(
  ...
  let f = if !no_reenter {
    let f = create_parse_function(
      args.j,
      args.m,
      &format!("reduce_{}", args.state_name),
    );
...
fn transfer_chkp_line_to_start_line(
  args: &BuildArgs,
  p_ctx: PointerValue,
) -> SherpaResult<()> {
  let LLVMParserModule { b, .. } = args.m;
...
fn transfer_start_line_to_end_line(
  args: &BuildArgs,
  p_ctx: PointerValue,
) -> SherpaResult<()> {
  let LLVMParserModule { b, .. } = args.m;
