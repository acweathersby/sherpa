...
impl ASTRefString {
  ...
  pub fn report<'a>(&self, source: &'a [u8]) -> String {
    ...
    if self.offset >= length {
      ...
      if let Ok(token) =
        str::from_utf8(&Vec::from(&source[self.offset..(self.offset + self.length)]))
      {
        let message = format!(
        ...
        let pointer_string = " ".repeat(self.offset - slice_start) + &("^".repeat(self.length));
...
pub type ReduceFunction_old<T: std::fmt::Debug> =
...
pub fn run<T: std::fmt::Debug>(
  ...
  reduce_functions: &[ReduceFunction_old<T>],
    ...
    match rule {
      0 => {
        ...
        if instr & 4 > 0 {
          ...
          len = if let Some(high_instr) = iter.next() { high_instr as usize } else { 0 };
        }

        let range = stack.drain((stack_pointer - len)..(stack_pointer)).collect();
      ...
      1 => {
        ...
        if instr & 4 > 0 {
          let high: usize =
            if let Some(high_instr) = iter.next() { high_instr as usize } else { 0 };

          len = ((len << 16) | high) as usize;
        }

        stack.push(Box::new(ASTRef::STRING(ASTRefString { offset: token_offset, length: len })));
...
mod completer_test {
  ...
  // }
  //
  // static D: [ReduceFunction_old<Test>; 1] = [
  ...
  // assert_eq!(std::str::from_utf8(_string.slice(&string)),
  // Ok(")*+")) }
  // }
  // }
  // }
