...
impl ByteReader for UTF8StringReader {
  ...
  fn next(&mut self, amount: u32) {
    ...
    if amount == 1 {
      ...
      let diff = std::cmp::max(std::cmp::min(4, (self.length - self.cursor) as i32), 0) as u32;
      ...
      for i in (self.cursor - amount as usize + 1)..std::cmp::min(self.length, self.cursor + 1) {
        let byte = self.string[i as usize];
