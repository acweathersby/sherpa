...
import {
    DebugIterator, DebugState, ParseActionType, StringByteReader
...
function hc_init(
    ...
    global_grammar_context = {
        ...
        active_tree: null,
        io: {
            entry_point: <HTMLSelectElement>document.getElementById("entrypoint-selector"),
...
function initParseContent() {
    const entry = parseInt(global_grammar_context.io.entry_point.selectedOptions[0].value);
...
function next() {
    if (global_grammar_context.parse_context) {
        ...
        if (prev_state.ACTIVE && current_state_index == states.length - 1) {
            ...
            let prev = current_state_index;

            while (states[prev].SCANNER)
                prev--;

            let prev_state = states[prev];

            let new_states = parser.next_state(prev_state);

            states.push(...new_states);

            let next_state = new_states[0];

            let stack = stacks[prev].slice();

            for (const _ of new_states)
                stacks.push(stack);
            ...
            for (const action of next_state.actions) {
                switch (action.type) {
                    ...
                    case ParseActionType.SHIFT: {
                        const { token: {
                            codepoint_length: length,
                            codepoint_offset: offset,

                            line_number,
                            line_offset
                        } } = action;
                        stack.push(input.slice(offset, offset + length) + `[@${line_number + 1}:${offset - line_offset}]`);
                    } break;
        ...
        } else if (prev_state.ACTIVE || prev_state.SCANNER) {
            global_grammar_context.parse_context.current_state_index = current_state_index + 1;
        }
    }
}
...
function renderState(
    ...
    center.innerHTML = createStateElement(curr_state.state, grammar_context) + (curr_state.fail_mode ? " fail" : "");
    ...
    for (const action of curr_state.actions) {
        switch (action.type) {
            ...
            case ParseActionType.SHIFT: {
                ...
                temp_text += `<div>SHIFT at offset ${offset} tok: [${tokens.get(type)}][${type}]</div>`;
...
function dis_address(count: number) {
    return ("000000000" + ((count & 0xFFFFFF) * 4).toString(10)).slice(-6);
}
